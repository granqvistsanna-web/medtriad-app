...
files_modified:
  - medtriad/app.json
  - medtriad/eas.json
  - medtriad/assets/images/icon.png
autonomous: true
...
must_haves:
  truths:
    ...
    - "App icon is 1024x1024 pixels for App Store submission"
  artifacts:
    ...
    - path: "medtriad/assets/images/icon.png"
      provides: "App icon at required 1024x1024 size"
      min_size: "1024x1024"
  key_links:
    ...
    - from: "medtriad/eas.json"
      ...
      pattern: "production"
    - from: "medtriad/app.json"
      to: "medtriad/assets/images/icon.png"
      via: "icon field references asset"
      pattern: "icon.*icon.png"
---

<objective>
Configure app.json and eas.json for App Store submission, including bundle identifier, privacy manifest, EAS build/submit profiles, and verify app icon meets 1024x1024 requirement.

Purpose: Technical configuration must be correct before any submission attempt
Output: app.json with iOS-specific settings, eas.json with production build and submit profiles, verified app icon
</objective>
...
<task type="auto">
  <name>Task 3: Verify app icon meets 1024x1024 requirement</name>
  <files>medtriad/assets/images/icon.png</files>
  <action>
Verify the app icon at medtriad/assets/images/icon.png meets App Store requirements:

1. Check icon dimensions using sips (macOS built-in):
   ```bash
   sips -g pixelWidth -g pixelHeight medtriad/assets/images/icon.png
   ```

2. Required dimensions: 1024x1024 pixels exactly
   - App Store requires this size for the App Store listing
   - Expo/EAS generates all other required sizes (180x180, 120x120, etc.) from this source

3. If icon is NOT 1024x1024:
   - Document the current size
   - Note in SUMMARY that user must provide a 1024x1024 icon before submission
   - The icon should be:
     - Square (1:1 aspect ratio)
     - PNG format
     - No transparency (App Store rejects icons with alpha)
     - No rounded corners (iOS applies masking automatically)

4. If icon IS 1024x1024:
   - Verify it has no transparency: `sips -g hasAlpha medtriad/assets/images/icon.png`
   - If hasAlpha is yes, note that user may need to flatten the image

Document findings for the SUMMARY.
  </action>
  <verify>Run `sips -g pixelWidth -g pixelHeight medtriad/assets/images/icon.png` and confirm dimensions are 1024x1024, or document the gap</verify>
  <done>App icon verified as 1024x1024, or gap documented with required action for user</done>
</task>
...
7. App icon is 1024x1024 pixels (or gap documented)
</verification>

<success_criteria>
- AS-01: App icon verified at 1024x1024 (or documented as needed)
