...
<task type="auto">
  <name>Task 3: Verify adaptive behavior with app startup</name>
  <files></files>
  <action>
Verify the integration works by running the app:
...
6. Observe console for "[Adaptive] Selected triads:" log (added in Plan 28-01)
7. Should see more questions from the weak category and tricky items
...
The dev logging in adaptive-selection.ts (from Plan 28-01) helps verify the adaptive path is executing during manual testing.
  </action>
  <verify>
```bash
cd medtriad && npx tsc --noEmit
...
- Adaptive selection code path executes (verified via __DEV__ console.log from Plan 28-01)
