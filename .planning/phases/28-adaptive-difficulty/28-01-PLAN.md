...
- If performance is null, return 'new'
- Calculate totalAttempts = correctCount + incorrectCount. If totalAttempts < 3, return 'new' (ADPT-05)
- Calculate accuracy = correctCount / totalAttempts
...
- If count exceeds items.length, return all items (edge case handling)
...
```
  ...
  <done>
- classifyDifficulty calculates totalAttempts from correctCount + incorrectCount
- classifyDifficulty returns 'new' for null or < 3 attempts
...
- weightedRandomSelect handles count > items.length edge case
...
- Returns 'new' when correctCount + incorrectCount < 3 (e.g., correctCount: 1, incorrectCount: 1)
- Returns 'new' when correctCount + incorrectCount === 2
- Returns 'easy' when accuracy >= 85% (e.g., correctCount: 9, incorrectCount: 1)
- Returns 'medium' when accuracy 51-84% (e.g., correctCount: 6, incorrectCount: 4)
- Returns 'hard' when accuracy <= 50% (e.g., correctCount: 4, incorrectCount: 6)
...
- Returns correct count when count < items.length
- Returns all items when count > items.length
...
- weightedRandomSelect edge case test passes
...
- classifyDifficulty correctly calculates totalAttempts from correctCount + incorrectCount
...
- weightedRandomSelect handles count > items.length edge case
- Dev logging included for verification during integration
